...
interface Estacionamento {
  id?: number; // Adicionado para vincular ao ID do backend
  nome: string;
  endereco: string;
  vagas: number;
  valorHora: number;
...
const TelaDono = () => {
  ...
  // Adicionar um novo estacionamento
  const adicionarEstacionamento = (novoEstacionamento: Estacionamento) => {
    const estacionamentoComConfirmacao = { ...novoEstacionamento, confirmado: false };
    setEstacionamentos([...estacionamentos, estacionamentoComConfirmacao]);
    setMostrarFormularioEstacionamento(false);
  };
  ...
  // Confirmar o estacionamento e navegar para a URL com o ID
  const confirmarEstacionamento = (index: number) => {
    const estacionamento = estacionamentos[index];
  ...
  // Alternar entre estacionamentos e funcionários
  const mostrarTelaEstacionamentos = () => setMostrarTabelaFuncionarios(false);
  const mostrarTelaFuncionarios = () => setMostrarTabelaFuncionarios(true);
  ...
  // Carregar estacionamentos do backend
  useEffect(() => {
    const carregarEstacionamentos = async () => {
      try {
        const usuarioId = 1; // ID do usuário (pode ser dinâmico)
        const dados = await buscarEstacionamentos(usuarioId);
        setEstacionamentos(
          dados.map(estacionamento => ({
            id: estacionamento.id, // Associando o ID do backend
            nome: estacionamento.estacionamentoNome,
            endereco: estacionamento.endereco,
            vagas: estacionamento.totalvagas,
            valorHora: estacionamento.valorHora,
            valorMaisHoras: estacionamento.valorMaisHoras,
            confirmado: false, // Estado inicial de confirmação
          }))
        );
      } catch (error) {
        console.error('Erro ao carregar estacionamentos:', error);
